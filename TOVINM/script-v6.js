...
function applyMobileScaleV6() {
  ...
  if (familyWrap && familyTree) {
    if (mobile) {
      const baseWidth = 1300;
      const scale = Math.min(1, familyWrap.clientWidth / baseWidth);
      familyTree.style.width = baseWidth + 'px';
      familyTree.style.transformOrigin = 'top left';
      familyTree.style.transform = `scale(${scale})`;
      requestAnimationFrame(() => {
        familyWrap.style.height = Math.ceil(familyTree.scrollHeight * scale) + 'px';
      });
    } else {
      familyTree.style.transform = '';
      familyWrap.style.height = '';
...
window.addEventListener('DOMContentLoaded', applyMobileScaleV6);
window.addEventListener('load', applyMobileScaleV6);
window.addEventListener('resize', applyMobileScaleV6);
setTimeout(applyMobileScaleV6, 400);
