...
function fitFamilyTree(){
  ...
  if(window.innerWidth<=1000){
    const available=Math.max(280,wrap.clientWidth);
    const base=1300;
    const scale=Math.min(1,available/base);
    tree.style.transform=`scale(${scale})`;
    wrap.style.height=`${Math.ceil(tree.offsetHeight*scale)}px`;
  }else{
    tree.style.transform='';
    wrap.style.height='';
  }
}
window.addEventListener('resize',fitFamilyTree);
window.addEventListener('load',fitFamilyTree);
setTimeout(fitFamilyTree,200);
